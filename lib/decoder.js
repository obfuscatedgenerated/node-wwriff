...
var Codebook = require('./codebook.js')
var fs = require('fs');
...
Wwriff_Decoder.prototype._read_data_chunk = function(chunk) {
    ...
    while(true) {
        ...
        if(!next_ogg_end) {
            ...
            //console.log(packet.next_offset(), packet.next_offset() + packet.header_size(), '>', chunk.length);
        ...
        //console.log(packet.offset(), next_packet.offset(), chunk.length);
        ...
        if(this._info.mode_blockflag[mode_number]) {
            ...
            if(this._total_offset + next_packet.next_offset() + next_packet.header_size() <= this._info.file_size) {
                if(next_packet.size() > 0) {
                    //console.log('seeking to', next_packet.offset());
        ...
        var bs = Math.pow(2, (this._info.mode_blockflag[mode_number] == 0) ? this._vorb.blocksize0_pow : this._vorb.blocksize1_pow);
